...
public class InwarehouseBoImpl extends WareHouseControlBoImpl implements InwarehouseBo {
	...
	public List<StorageIoDetail> addListInwarehouseOperate(StorageIoBatch storageIoTask,
		...
		for (StorageIoDetail storageIoDetail : listDetails) {
			...
			ioAmount += addOneInwarehouseOperate(storageIoTask.getTaskId(), storageIoDetail.getSkuId(),
					storageIoDetail.getAmount(), storageIoDetail.getPositionId());
		}
		// 记录总量修改 complete
	...
	public int addOneInwarehouseOperate(Integer taskId, Integer skuId, Integer amount, Integer positionId) {
		...
		if (StringUtils.isBlank(failReason)) {// 能入库
			disAmount = addInwarehouseOperate(taskId, skuId, amount, positionId,
					ConstantStorageIoDetailIoDetailType.IN_WAREHOUSE);
		} else {// 不能入库抛出失败原因
	...
	private String findFailIoDetail(Integer skuId, Integer amount, Integer positionId) {
		...
		StoragePosition position = storagePositionDao.findOne(positionId);
		if (position == null) {// 无库位不能入库
			return "库位不存在！！";
		}
		if (position.getIslock() == true) {// 已锁定不能入库
			return position.getLockReason();
		}
		return StringUtils.EMPTY;
	}

	public int addInwarehouseOperate(Integer taskId, Integer skuId, Integer amount, Integer positionId,
			String ioDetailType) {
		// 添加变动单明细<>
		this.addStorageIoTaskDetail(taskId, positionId, skuId, amount, IoType.IN);
		// 添加或更新总库存
		StorageProduct product = this.findStorageProductBySkuId(skuId);
		if (product == null) {
			// 库存无记录则添加
	...
	public StoragePosition addStoragePosition(String positionLabel, Integer capacity, String memo) {
		...
		StoragePosition position = this.storagePositionDao.findByLabel(positionLabel);
		if (position != null) {
			throw new RuntimeException("当前库位已存在！！");
		}
		StoragePosition instance = new StoragePosition();
		instance.setLabel(positionLabel);
