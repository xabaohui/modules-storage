...
public class StorageOperaterServiceImpl implements 	StorageOperaterService {
	...
	public DistrBatchOutOfStockResponseDTO firstDistriApply(DistrBatchRequestDTO requestDto) {
		...
		StorageIoTask task = createStorageIoTask(requestDto.getOperateor(),requestDto.getMemo(),ConstantStorageIoTaskBizType.OUT_WAREHOUSE);
		...
		try {
			for (DistrBatchRequestDetail requestDetail : listRequestDetails) {
				...
				disAmount += outwarehouseBo.addOutwarehouseOperate(
						task, requestDetail.getSkuId(), requestDetail.getSkuCount());
			}
	...
	public DistrBatchOutOfStockResponseDTO errorOutDetailFill(DistrBatchOutOfStockRequestDTO requestDto) {
		...
		for (DistrBatchOutOfStockRequestDetail requestDetail : listreRequestDetails) {
			list.add(new DistrBatchRequestDetail(requestDetail.getSkuId(),
					requestDetail.getSkuCount()));
		}
		DistrBatchRequestDTO request = new DistrBatchRequestDTO(
				requestDto.getOperator(), "缺货补配", list);
		//缺货补配先占用库存
	...
	public void occupyStorageProduct(List<DistrBatchRequestDetail> list) {
		for (DistrBatchRequestDetail instance : list) {
			outwarehouseBo.updateStorageProductOccupyStock(instance.getSkuId(),instance.getSkuCount());
		}
	}
	...
	public void removeOccupyStorageProduct(List<DistrBatchRequestDetail> list) {
		for (DistrBatchRequestDetail instance : list) {
			outwarehouseBo.updateStorageProductRemoveOccupyStorage(instance.getSkuId(),instance.getSkuCount());
		}
	}
	 ...
	 * 创建变动单
	 */
	public StorageIoTask createStorageIoTask(Integer operator,String memo,String operateType) {
		...
		if(operateType!=ConstantStorageIoTaskBizType.IN_WAREHOUSE&&operateType!=ConstantStorageIoTaskBizType.OUT_WAREHOUSE){
			throw new RuntimeException("变动单类型不正确");
		}
		return inwarehouseBo.addStorageIoTask(operator, 0,memo,operateType);
	}
	...
	public StorageCheckPlan createStorageCheckPlan(Integer operator,String memo) {
	  ...
	  *  创建盘点
	  */
	public StorageCheck createStorageCheck(Integer planId, Integer operator,
	 ...
	 * 第一次盘点
	 */
	public List<StorageCheckDiffReaultDetail> firstStorageCheck(
	 ...
	 * 第二次盘点
	 */
	public List<StorageCheckDiffReaultDetail> secondStorageCheck(
			List<StorageCheckSnapDataDetail> list, Integer checkId) {
		
		checkStockBo.addCheckSnapData(list, checkId, ConstantStorageCheckCheckTime.SECOND_CHECK);
		List<StorageCheckDiffReaultDetail> listDetails = checkStockBo.compareDiff(checkId, ConstantStorageCheckCheckTime.SECOND_CHECK);
		if(listDetails==null){
	 ...
	 * 库存调整<单个>
	 */
	public void adjustMentCheckResult(Integer operator,String memo,Integer checkId ,
					Integer skuId, Integer realAmount) {
